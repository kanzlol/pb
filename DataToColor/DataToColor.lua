...
function DataToColor:CreateFrames(n)
    ...
    local function UpdateFrameColor(f)
        ...
        if not SETUP_SEQUENCE then
            ...
            MakePixelSquareArr(integerToColor(self:GetBuffsForClass()), 41) --class buffs (only warlock so far)
            MakePixelSquareArr(integerToColor(self:playerCombatStatus()), 42) --player in combat?
...
function DataToColor:GetBuffsForClass()
    ...
    if CC == "WARLOCK" then
        class = self:MakeIndexBase2(self:GetBuffs("Demon Skin"), 0)
    end
